...
use matrix_sdk::{
    ...
    ruma::{
        MilliSecondsSinceUnixEpoch, OwnedMxcUri, OwnedRoomAliasId, OwnedRoomId, events::tag::Tags,
    },
};
...
impl RoomsList {
    ...
    fn update_frontend_state(&self) {
        self.state_updaters
            .update_rooms_list(self)
            .expect("Couldn't update the frontend RoomsList state")
    }
                ...
                RoomsListUpdate::AddJoinedRoom(joined_room) => {
                    ...
                    if let Some(_old_room) = _replaced {
                        ...
                        if should_display {
                            ...
                            send_room_creation_request_and_await_response(room_id.as_str())
                                .await
