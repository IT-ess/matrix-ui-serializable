...
pub async fn add_new_room(
    ...
    println!(
        "Adding new joined room {room_id}. Replaces tombstoned room: {tombstoned_room_replaced_by_this_room:?}"
    );
    ...
    if old_room.room_id == new_room_id {
        ...
        if old_room_state != new_room_state {
            if LOG_ROOM_LIST_DIFFS {
                println!(
                    "Room {new_room_name:?} ({new_room_id}) changed from {old_room_state:?} to {new_room_state:?}"
                );
            }
